---
title: "From Recommendations For Better Nutrition and More Exercise to Recommendations Regarding the Side Effects of Vital Medicines. Benefits and Data Protection in Different Application Contexts of Health Recommendation Systems"
author: "Laura"
date: "24 4 2019"
...
raw <- haven::read_sav("Data/DatenRecSys2019.sav", encoding = "UTF-8")


raw %>% select(AllgemeineDaten1, AllgemeineDaten2, AllgemeineDaten3, GesundheitsbezogeneDaten1, GesundheitsbezogeneDaten2, GesundheitsbezogeneDaten3) -> df



df_test <- raw %>% select(respondent_id, AllgemeineDaten1, AllgemeineDaten2, AllgemeineDaten3, GesundheitsbezogeneDaten1, GesundheitsbezogeneDaten2, GesundheitsbezogeneDaten3)
...
```{r data-cleaning2, include=FALSE}
(
p <- 
    df %>% 
    psych::describe() %>% 
    ...
    left_join(wsci_df) %>% 
    mutate(ci = value) %>% 
    mutate(data_type = ifelse(str_detect(variable, "Allgemeine"), "personal data", "health data")) %>% 
...
g
ggsave("plot.pdf", g, width = 5, height = 3.5)  
...
(p <- df %>% 
  ...
  mutate(data_type = case_when(str_detect(attribute,"gender") ~ "personal data",
                               str_detect(attribute,"day of birth") ~ "personal data",
                               str_detect(attribute,"height") ~ "personal data",
                               str_detect(attribute,"weight") ~ "personal data",
                               TRUE ~ "health data")
        ) %>% 
...
g
ggsave("plot2.pdf", g, width = 5, height = 3.75)  
...
g
ggsave("plot3.pdf", g, width = 5, height = 3.75)  
```
